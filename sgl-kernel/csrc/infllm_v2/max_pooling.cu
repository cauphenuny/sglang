// InfLLM-V2 max pooling (varlen + fixed-length), AOT build.
//
...
// input:  [num_heads, q_len, k_len]
// output: [num_heads, q_len, out_len]
template <typename T>
__global__ void max_pooling_1d_kernel(
  ...
  const T* in = input + static_cast<size_t>(bidh) * q_len * k_len + static_cast<size_t>(bidq) * k_len;
  T* out = output + static_cast<size_t>(bidh) * q_len * out_len + static_cast<size_t>(bidq) * out_len;

  const int off_bq = (bidq + cache_len) / block_size;
  const T pos_inf = static_cast<T>(static_cast<float>(INFINITY));
...
void infllm_v2_max_pooling_1d(
    ...
    int64_t local_blocks,
    int64_t init_blocks) {
  TORCH_CHECK(input.dim() == 3, "input must be 3D [num_heads, q_len, k_len]");
  TORCH_CHECK(output.dim() == 3, "output must be 3D [num_heads, q_len, out_len]");

  const int num_heads = static_cast<int>(input.size(0));
  const int q_len = static_cast<int>(input.size(1));
  const int k_len = static_cast<int>(input.size(2));
  const int out_len = static_cast<int>(output.size(2));

  const cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  const dim3 grid(q_len, num_heads);
  const dim3 block(256);

  DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(input.scalar_type(), c_type, [&] {
    max_pooling_1d_kernel<c_type><<<grid, block, 0, stream>>>(
        static_cast<const c_type*>(input.data_ptr()),
        static_cast<c_type*>(output.data_ptr()),
        num_heads,
        q_len,
