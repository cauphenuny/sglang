...
def test_write_mha_kv_cache_preserves_locations_and_scales():
    backend = _backend()
    backend.token_to_kv_pool = SimpleNamespace(set_kv_buffer=Mock())
    layer = SimpleNamespace()
    cache_loc = torch.tensor([1, 2])
    ...
    args = backend.token_to_kv_pool.set_kv_buffer.call_args.args
    assert args[0] is layer
    assert args[1].loc is cache_loc
    assert args[1].swa_loc is swa_loc
    assert args[2] is k
    assert args[3] is v
    assert args[4] is k_scale
    assert args[5] is v_scale
