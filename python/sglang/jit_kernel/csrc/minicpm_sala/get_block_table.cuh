...
__global__ void get_block_table_cuda_v1(const int* topk_idx, const int* block_table, const int* token_to_bs,
  ...
  for (int h = 0; h < kHeadGroup; h++) {
    for (int i = 0; i < kSparseTopK * kSparseBlockSize; i++) {
      int sparse_block_idx =
          topk_idx[h * token_num * kSparseTopK + token_idx * kSparseTopK + i / kSparseBlockSize];
...
template <int kSparseTopK>
__global__ void get_block_table_cuda_v2(const int* topk_idx, const int* block_table, const int* token_to_bs,
                                        const int* token_pos_in_bs, const int* seqlen_q, int* out_block_table,
                                        const int seqlen_q_max, const int token_num) {
  ...
  for (int i = 0; i < kSparseBlockSize; i++) {
    ...
    if (token_idx_in_batch < seqlen_q_bs && token_idx_in_batch < pos_in_bs) {
      out_block_table[token_idx * kHeadGroup * kSparseTopK * kSparseBlockSize +
                      head_group_idx * kSparseTopK * kSparseBlockSize + topk_idx_in_head * kSparseBlockSize + i] =
          kHeadGroup * block_table[bs * seqlen_q_max + token_idx_in_batch] + head_group_idx;
    } else {
      out_block_table[token_idx * kHeadGroup * kSparseTopK * kSparseBlockSize +
                      head_group_idx * kSparseTopK * kSparseBlockSize + topk_idx_in_head * kSparseBlockSize + i] = 0;
    }
  }
}
...
// smem 1024 / 64 = 16.
template <int kSparseTopK>
__global__ void get_block_table_cuda_v3(const int* topk_idx, const int* block_table, const int* token_to_bs,
                                        const int* token_pos_in_bs, const int* seqlen_q, int* out_block_table,
                                        const int seqlen_q_max, const int token_num) {
...
template <int kSparseTopK>
void verify_inputs(tvm::ffi::TensorView out, tvm::ffi::TensorView topk_idx, tvm::ffi::TensorView block_table,
                   tvm::ffi::TensorView token_to_bs, tvm::ffi::TensorView token_pos_in_bs,
                   tvm::ffi::TensorView seqlen_q, host::SymbolicSize& token_num, host::SymbolicSize& batch_size,
                   host::SymbolicSize& seqlen_q_max, host::SymbolicDevice& device) {
...
template <int kSparseTopK>
void get_block_table_v1(tvm::ffi::TensorView out, tvm::ffi::TensorView topk_idx, tvm::ffi::TensorView block_table,
                        tvm::ffi::TensorView token_to_bs, tvm::ffi::TensorView token_pos_in_bs,
  ...
  verify_inputs<kSparseTopK>(out, topk_idx, block_table, token_to_bs, token_pos_in_bs, seqlen_q, token_num,
                             batch_size, seqlen_q_max, device);
  ...
  LaunchKernel(num_blocks, kThreadsPerBlock, dev)(
      get_block_table_cuda_v1<kSparseTopK>, static_cast<const int*>(topk_idx.data_ptr()),
      static_cast<const int*>(block_table.data_ptr()), static_cast<const int*>(token_to_bs.data_ptr()),
      static_cast<const int*>(token_pos_in_bs.data_ptr()), static_cast<const int*>(seqlen_q.data_ptr()),
      static_cast<int*>(out.data_ptr()), s_q_max, n_token);
}

template <int kSparseTopK>
void get_block_table_v2(tvm::ffi::TensorView out, tvm::ffi::TensorView topk_idx, tvm::ffi::TensorView block_table,
                        tvm::ffi::TensorView token_to_bs, tvm::ffi::TensorView token_pos_in_bs,
                        tvm::ffi::TensorView seqlen_q) {
  using namespace host;
  SymbolicSize token_num{"token_num"}, batch_size{"batch_size"}, seqlen_q_max{"seqlen_q_max"};
  ...
  LaunchKernel(num_blocks, kThreadsPerBlock, dev)(
      get_block_table_cuda_v2<kSparseTopK>, static_cast<const int*>(topk_idx.data_ptr()),
      static_cast<const int*>(block_table.data_ptr()), static_cast<const int*>(token_to_bs.data_ptr()),
      static_cast<const int*>(token_pos_in_bs.data_ptr()), static_cast<const int*>(seqlen_q.data_ptr()),
      static_cast<int*>(out.data_ptr()), s_q_max, n_token);
}
...
template <int kSparseTopK>
void get_block_table_v3(tvm::ffi::TensorView out, tvm::ffi::TensorView topk_idx, tvm::ffi::TensorView block_table,
                        tvm::ffi::TensorView token_to_bs, tvm::ffi::TensorView token_pos_in_bs,
                        tvm::ffi::TensorView seqlen_q) {
  using namespace host;
  SymbolicSize token_num{"token_num"}, batch_size{"batch_size"}, seqlen_q_max{"seqlen_q_max"};
  ...
  constexpr int kThreadsPerBlock = 1024;
  const int64_t total =
      static_cast<int64_t>(n_token) * kHeadGroup * kSparseTopK * kSparseBlockSize;
  const int64_t num_blocks = (total + kThreadsPerBlock - 1) / kThreadsPerBlock;

  LaunchKernel(num_blocks, kThreadsPerBlock, dev)(
      get_block_table_cuda_v3<kSparseTopK>, static_cast<const int*>(topk_idx.data_ptr()),
      static_cast<const int*>(block_table.data_ptr()), static_cast<const int*>(token_to_bs.data_ptr()),
      static_cast<const int*>(token_pos_in_bs.data_ptr()), static_cast<const int*>(seqlen_q.data_ptr()),
      static_cast<int*>(out.data_ptr()), s_q_max, n_token);
}
