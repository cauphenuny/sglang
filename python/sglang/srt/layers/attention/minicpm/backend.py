...
class MiniCPMSparseBackend(AttentionBackend):
    ...
    def __init__(
        ...
        self.has_minicpm_sparse_attention = hf_config is not None and getattr(
            hf_config, "has_minicpm_sparse_attention", False
        )

        # MiniCPM must have sparse attention enabled
        if not self.has_minicpm_sparse_attention:
            raise ValueError(
    ...
    def _replay_cuda_graph_metadata(
        ...
        if forward_mode.is_decode_or_idle():
            ...
            decode_metadata, compression_metadata = (
                self._build_sparse_decode_replay_metadata(forward_batch, metadata)
            )
